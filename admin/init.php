...
// Path to the admin folder
define('ABSBOPATH', str_replace('\\', '/', realpath('./')));
...
$db = Database::Instance(); // Database class instanciation
...
// DB Connect
$db->connect( $Serveur = DB_HOST , $User = DB_USER, $Password = DB_PASSWORD, $Database = DB_NAME, $Prefix = DB_PREFIX );

// OutPutDebug class instanciation
$debug	= new debug();
...
/**
  ...
  function get_footer( $filename = 'footer.php' ) {
	  ...
	  if ( is_file( $filename ) AND isset( TPL . $filename ) ) {
